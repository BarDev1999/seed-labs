...
Client A's ARP cache was cleared first. The entry for `10.0.2.8` appeared as `(incomplete)`:

![Client ARP table before attack: 10.0.2.8 is (incomplete), no valid MAC entry](assets/screenshot-07.png)

The Reply was sent from the Attacker:

![Attacker terminal: "SENDING SPOOFED ARP REPLY...... Sent 1 packets."](assets/screenshot-08.png)

After the attack, Client A's ARP table was verified:

![Client ARP table after attack: 10.0.2.8 mapped to 08:00:27:36:98:8e (Attacker MAC), changed from incomplete](assets/screenshot-07.png)
...
> **Note:** ICMP Redirect messages from the Attacker machine (10.0.2.6) are automatically generated by the Linux kernel because it is routing traffic back out the same interface it arrived on. The continuous ARP poisoning overrides these redirect attempts and maintains the MITM position.
